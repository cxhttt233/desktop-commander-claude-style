...
const testTaskName = data.tasks.find((x) => x.calls === 3)?.name;
assert(testTaskName, 'test task should be present');
assert(data.dimensions.tools.some((x) => x.name === 'read_file'));
assert(data.dimensions.tasks.some((x) => x.name === testTaskName));
assert.equal(data.scope.calls, 6);
const filteredTask = await fetch('http://127.0.0.1:17991/api/summary?days=7&task=' + encodeURIComponent(testTaskName)).then((r) => r.json());
assert.equal(filteredTask.range.calls, 3, 'task filter should aggregate the selected task only');
const filteredTool = await fetch('http://127.0.0.1:17991/api/summary?days=7&tool=read_file&task=' + encodeURIComponent(testTaskName)).then((r) => r.json());
assert.equal(filteredTool.range.calls, 1, 'tool + task filters should intersect');
const filteredError = await fetch('http://127.0.0.1:17991/api/summary?days=7&status=error').then((r) => r.json());
assert.equal(filteredError.range.calls, 1, 'status filter should isolate failures');
