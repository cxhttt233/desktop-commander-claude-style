...
At every start, the dashboard performs an idempotent best-effort history reconciliation. It reads `~/.claude-server-commander/tool-history.jsonl` for recent calls with outputs/duration and `dcTokenMeter`, plus `claude_tool_call*.log` for older input-side calls. Stable event IDs prevent already-recorded calls from being counted twice; older capped-history rows without a meter are matched against live rows by tool, timing, duration, and input size. Newly discovered completed calls can therefore be recovered after a later gateway restart. The reconciliation state is written atomically to `history-recovery-v2.json`. A tool call that was still in flight when the main Desktop Commander process itself died, and never reached either the live stats log or tool history, cannot be reconstructed.
